...
const DiscordSetup = () => {
  ...
  const [addToServer, setAddToServer] = useState(false);
  ...
  useEffect(() => {
    const fetchDataSetup = async () => {
      try {
        ...
        console.error('Error fetching data setup:', error);
      }
    };
  
    const fetchMutedRoleId = async () => {
      try {
        const response = await axios.get('http://localhost:8081/moderation-setup');
        setMutedRoleId(response.data.muted_role || '');
      } catch (error) {
    ...
    const fetchCommunityRules = async () => {
      try {
        ...
        setCommunityRules(response.data.content || '');
      } catch (error) {
        console.error('Error fetching community rules:', error);
      }
    };
  
    const fetchAddToServerStatus = async () => {
      try {
        const response = await axios.get('http://localhost:8081/add-to-server');
        setAddToServer(response.data.status);
      } catch (error) {
    ...
    fetchStaffList();
    fetchCommunityRules();
    fetchAddToServerStatus();
  }, []);
  

  const handleMutedRoleIdChange = async (e) => {
    const newMutedRoleId = e.target.value;
    setMutedRoleId(newMutedRoleId);

    try {
      await axios.post('http://localhost:8081/moderation-setup', {
        muted_role: newMutedRoleId,
      });
    } catch (error) {
      console.error('Error updating muted role ID:', error);
    }
  };

  const handleAddToServerChange = async (e) => {
    const newStatus = e.target.checked;
    setAddToServer(newStatus);

    try {
      await axios.post('http://localhost:8081/add-to-server', {
        status: newStatus,
      });
    } catch (error) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      await axios.post('http://localhost:8081/data-setup', settings);
  
      if (staffList) {
        await axios.post('http://localhost:8081/staff-list', { content: staffList });
      }
  
      if (communityRules) {
        await axios.post('http://localhost:8081/community-rules', { content: communityRules });
      }
  ...
  return (
    <div style={{ ...styles.container }}>
      ...
      <TextField
        ...
        variant="outlined"
        value={mutedRoleId}
        onChange={handleMutedRoleIdChange}
        fullWidth
        margin="normal"
        style={styles.textField}
      />
      ...
      <TextField
        ...
        helperText={`${staffList.length}/4000 characters`}
        style={styles.textField}
      />
      ...
      <TextField
        ...
        helperText={`${communityRules.length}/8000 characters`}
        style={styles.textField}
      />
      ...
      <FormControlLabel
        control={
          <Checkbox
            checked={addToServer}
            onChange={handleAddToServerChange}
            style={styles.checkbox}
          />
        }
        label="Add users to community server"
