...
const PositionInfo = () => {
  ...
  useEffect(() => {
    const fetchPositionInfo = async () => {
      if (positionName) {
        try {
          const response = await axios.get(`http://localhost:8081/position-info/${positionName}`);
    ...
    const checkCooldown = async () => {
      ...
      try {
        const response = await axios.get(`http://localhost:8081/waiting-time/${username}`);
        if (response.data.timeLeft !== 'No waiting time') {
          setIsCooldown(true);
          setCooldownMessage(`You're on cooldown! You can apply again ${response.data.timeLeft}.`);
        } else {
          setIsCooldown(false);
        }
      } catch (error) {
  ...
  const handleApply = async () => {
    try {
      ...
      const requestBody = {
        ...
        application_author: username,
      };
      await axios.post(`http://localhost:8081/apply-position/${positionName}`, requestBody);
  
      await axios.post(`http://localhost:8081/waiting-time/${username}`);
  
      await checkCooldown();
  
      alert('Application submitted successfully!');
      setResponses(positionDetails.questions.map(() => ''));
    } catch (error) {
  ...
  return (
    <div style={{ ...styles.container }}>
      ...
      {isCooldown ? (
        <Alert severity="warning">{cooldownMessage}</Alert>
      ) : !positionDetails ? (
        ...
        <div>
          <div style={{ textAlign: 'center', marginBottom: '20px' }}>
            ...
            <Typography variant="body1" component="p">
              {positionDetails.position_description || 'No description available.'}
            </Typography>
          </div>
