...
const UserManagement = () => {
  ...
  const handleViewBannedUsers = async () => {
    try {
      const response = await axios.get("http://localhost:8081/banned-users");
      const bannedUserIds = response.data;
  
      const bannedUsersWithNames = await Promise.all(bannedUserIds.map(async (userId) => {
        try {
          const usernameResponse = await fetchDiscordUsername(userId);
          return { userId, username: `${usernameResponse.username} (${userId})` };
        } catch (error) {
          console.error(`Error fetching username for user ID ${userId}:`, error);
          return { userId, username: `Unknown (${userId})` };
        }
      }));
  
      setBannedUsers(bannedUsersWithNames);
      setOpenBannedUsersDialog(true);
    } catch (error) {
  ...
  const fetchDiscordUsername = async (userId) => {
    try {
      const response = await axios.get(`http://localhost:8081/discord-username/${userId}`);
      return response.data;
    } catch (error) {
      console.error(`Error fetching Discord username for ID ${userId}:`, error);
      throw error;
    }
  };
  
  const setBanUserID = async () => {
    try {
      const username = await fetchDiscordUsername(userId);
  
      if (!username) {
        return;
      }
  
      await axios.post("http://localhost:8081/ban-user", { userId });
    } catch (error) {
  ...
  const handleUnbanUser = async () => {
    try {
      ...
      await axios.post("http://localhost:8081/unban-user", { userId });
    } catch (error) {
  ...
  const handleCloseBannedUsersDialog = () => {
    setOpenBannedUsersDialog(false);
  };
  ...
  return (
    <div style={{ ...styles.container }}>
      ...
      <div style={styles.whiteContainer}>
        ...
        <Button variant="contained" onClick={handleViewBannedUsers}>
          View Banned Users
        </Button>
  ...
  <DialogContent>
    <ul>
      {bannedUsers.map((user) => (
        <li key={user.userId}>{user.username || 'Unknown'}</li>
      ))}
    </ul>
  </DialogContent>
  <DialogActions>
    <Button onClick={handleCloseBannedUsersDialog} color="primary">Close</Button>
  </DialogActions>
</Dialog>
      </div>
      <Snackbar
