...
const Settings = () => {
  ...
  const handleCardClick = (positionName, status) => {
    localStorage.setItem('positionName', positionName);
    if (status === 'open') {
      navigate('/position_info');
    } else {
  ...
  <Grid container spacing={3}>
    {positions.map((position) => (
      <Grid item xs={12} sm={6} md={4} key={position.position_id}>
        <Card
          style={styles.card}
          onClick={() => handleCardClick(position.position_name, position.status)}
        >
          {position.position_image && (
            <CardMedia
              style={styles.media}
              image={position.position_image}
              title={position.position_name}
            />
          )}
          <CardContent style={styles.cardContent}>
            <Typography style={styles.textPosition} variant="h6" component="div">
            ...
            <div style={{ display: 'flex', alignItems: 'center' }}>
              <div
                style={{
                  ...styles.statusDot,
                  backgroundColor: position.status === 'open' ? 'green' : 'red',
                }}
              />
              <Typography variant="body2" component="div">
                {position.status === 'open' ? 'This position is open' : 'This position is closed'}
              </Typography>
            </div>
          </CardContent>
        </Card>
